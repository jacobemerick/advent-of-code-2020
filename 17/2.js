  ...
  for (let z = 0; z < state.length + 2; z++) {
    ...
    for (let y = 0; y < state[0].length + 2; y ++) {
      ...
      for (let x = 0; x < state[0][0].length + 2; x++) {
        newState[z][y].push([]);
        for (let w = 0; w < state[0][0][0].length + 2; w++) {
          const currentVal = state?.[z - 1]?.[y - 1]?.[x - 1]?.[w - 1] || '.';
          ...
          for (let oZ = -1; oZ < 2; oZ++) {
            for (let oY = -1; oY < 2; oY++) {
              for (let oX = -1; oX < 2; oX++) {
                for (let oW = -1; oW < 2; oW++) {
                  if (oZ === 0 && oY === 0 && oX === 0 && oW === 0) continue;
                  if (state?.[z + oZ - 1]?.[y + oY - 1]?.[x + oX - 1]?.[w + oW - 1] === '#') {
                    neighbors++;
                  }
                }
              }
...
let count = 0;
state.forEach(zyxState => {
  zyxState.forEach(yxState => {
